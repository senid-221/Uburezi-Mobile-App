...
type Screen = 'home' | 'lessons' | 'parent' | 'age' | 'auth' | 'children';
...
export default function App() {
  ...
  if (screen === 'parent') return <Shell><ParentDashboardScreen onBack={goHome} parent={parent} onLogin={() => setScreen('auth')} onChildren={() => setScreen('children')} /></Shell>;
  ...
  return <SafeAreaView style={styles.safe}><StatusBar style="dark" /><ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
    <View style={styles.header}><View><Text style={styles.eyebrow}>UBUREZI</Text><Text style={styles.title}>Murakaza neza! 👋</Text><Text style={styles.subtitle}>Kwiga, gukina no gutera imbere.</Text></View><TouchableOpacity onPress={openParent} style={styles.avatar}><Ionicons name="person" size={24} color="#2563EB" /></TouchableOpacity></View>
    ...
    <TouchableOpacity style={styles.parentButton} onPress={openParent}><Ionicons name="shield-checkmark-outline" size={19} color="#2563EB" /><Text style={styles.parentText}>Ahagana ku babyeyi</Text></TouchableOpacity>
